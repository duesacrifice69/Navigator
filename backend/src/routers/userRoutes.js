...
const { permissionsData, generateToken}  = require("../controllers/permissionController");
...
const 
  financialDataContoller
 = require("../controllers/financialDataContoller");
...
router.get("/permissions/authnew", auth, async (req, res) => {
  const permissions=permissionController.permission(permissionsData[req.userRole]);
  res.send({ permissions });
});
router.get("/registered-details",auth, async (req, res) => {
...
router.get("/events",async(req,res)=>{
  eventController.filterEvent(req,res);
});
